...
NSString * const kNEBLApplicationJSONType = @"application/json";

NSString * NEBLPercentEscapedStringFromString(NSString *string) {
    static NSString * const kNEBLCharactersGeneralDelimitersToEncode = @":#[]@";
    static NSString * const kNEBLCharactersSubDelimitersToEncode = @"!$&'()*+,;=";

    NSMutableCharacterSet * allowedCharacterSet = [[NSCharacterSet URLQueryAllowedCharacterSet] mutableCopy];
    [allowedCharacterSet removeCharactersInString:[kNEBLCharactersGeneralDelimitersToEncode stringByAppendingString:kNEBLCharactersSubDelimitersToEncode]];
...
- (id) sanitizeForSerialization:(id) object {
    ...
    else if ([object isKindOfClass:[NSString class]] || [object isKindOfClass:[NSNumber class]] || [object isKindOfClass:[NEBLQueryParamCollection class]]) {
        return object;
    }
    else if ([object isKindOfClass:[NSDate class]]) {
        return [NEBLSanitizer dateToString:object];
    }
    ...
    else if ([object isKindOfClass:[NEBLObject class]]) {
...
- (NSString *) parameterToString:(id)param {
    if ([param isKindOfClass:[NSString class]]) {
        ...
        return [NEBLSanitizer dateToString:param];
    }
...
+ (NSString *)dateToString:(id)date {
    NSTimeZone* timeZone = [NEBLDefaultConfiguration sharedConfig].serializationTimeZone;
...
- (NSString *) selectHeaderAccept:(NSArray *)accepts {
    ...
    for (NSString *string in accepts) {
        if ([self.jsonHeaderTypeExpression matchesInString:string options:0 range:NSMakeRange(0, [string length])].count > 0) {
            return kNEBLApplicationJSONType;
        }
