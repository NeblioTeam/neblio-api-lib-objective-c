...
#import "NEBLJSONRequestSerializer.h"
#import "NEBLQueryParamCollection.h"
#import "NEBLDefaultConfiguration.h"

NSString *const NEBLResponseObjectErrorKey = @"NEBLResponseObject";

static NSString * const kNEBLContentDispositionKey = @"Content-Disposition";

static NSDictionary * NEBL__headerFieldsForResponse(NSURLResponse *response) {
...
static NSString * NEBL__fileNameForResponse(NSURLResponse *response) {
    NSDictionary * headers = NEBL__headerFieldsForResponse(response);
    if(!headers[kNEBLContentDispositionKey]) {
    ...
    NSString *contentDispositionHeader = headers[kNEBLContentDispositionKey];
...
@interface NEBLApiClient ()

@property (nonatomic, strong, readwrite) id<NEBLConfiguration> configuration;

@property (nonatomic, strong) NSArray<NSString*>* downloadTaskResponseTypes;

@end

@implementation NEBLApiClient

#pragma mark - Singleton Methods

+ (instancetype) sharedClient {
    static NEBLApiClient *sharedClient = nil;
...
- (instancetype)init {
    return [self initWithConfiguration:[NEBLDefaultConfiguration sharedConfig]];
}

- (instancetype)initWithBaseURL:(NSURL *)url {
    return [self initWithBaseURL:url configuration:[NEBLDefaultConfiguration sharedConfig]];
}

- (instancetype)initWithConfiguration:(id<NEBLConfiguration>)configuration {
    return [self initWithBaseURL:[NSURL URLWithString:configuration.host] configuration:configuration];
}

- (instancetype)initWithBaseURL:(NSURL *)url configuration:(id<NEBLConfiguration>)configuration {
    self = [super initWithBaseURL:url];
    if (self) {
        _configuration = configuration;
        _timeoutInterval = 60;
        _responseDeserializer = [[NEBLResponseDeserializer alloc] init];
        _sanitizer = [[NEBLSanitizer alloc] init];

        _downloadTaskResponseTypes = @[@"NSURL*", @"NSURL"];

        AFHTTPRequestSerializer* afhttpRequestSerializer = [AFHTTPRequestSerializer serializer];
        NEBLJSONRequestSerializer * swgjsonRequestSerializer = [NEBLJSONRequestSerializer serializer];
        _requestSerializerForContentType = @{kNEBLApplicationJSONType : swgjsonRequestSerializer,
...
- (NSURLSessionDataTask*) taskWithCompletionBlock: (NSURLRequest *)request completionBlock: (void (^)(id, NSError *))completionBlock {
    ...
    NSURLSessionDataTask *task = [self dataTaskWithRequest:request uploadProgress:nil downloadProgress:nil completionHandler:^(NSURLResponse * _Nonnull response, id  _Nullable responseObject, NSError * _Nullable error) {
        NEBLDebugLogResponse(response, responseObject,request,error);
        if(!error) {
        ...
        if (responseObject) {
            ...
            userInfo[NEBLResponseObjectErrorKey] = responseObject;
        }
...
- (NSURLSessionDataTask*) downloadTaskWithCompletionBlock: (NSURLRequest *)request completionBlock: (void (^)(id, NSError *))completionBlock {
    ...
    NSURLSessionDataTask* task = [self dataTaskWithRequest:request uploadProgress:nil downloadProgress:nil completionHandler:^(NSURLResponse *response, id responseObject, NSError *error) {
        ...
        NSString *filename = NEBL__fileNameForResponse(response);
...
- (NSURLSessionTask*) requestWithPath: (NSString*) path
    ...
    __weak id<NEBLSanitizer> sanitizer = self.sanitizer;
    ...
    [pathParams enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        ...
        safeString = NEBLPercentEscapedStringFromString(safeString);
...
- (NSString*) pathWithQueryParamsToString:(NSString*) path queryParams:(NSDictionary*) queryParams {
    ...
    for(NSString * key in [queryParams keyEnumerator]){
        ...
        NSString *safeKey = NEBLPercentEscapedStringFromString(key);
        if ([queryParam isKindOfClass:[NSString class]]){
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, NEBLPercentEscapedStringFromString(queryParam)]];

        } else if ([queryParam isKindOfClass:[NEBLQueryParamCollection class]]){
            NEBLQueryParamCollection * coll = (NEBLQueryParamCollection*) queryParam;
            ...
            if([format isEqualToString:@"multi"]) {
                for(id obj in values) {
                    ...
                    NSString * safeValue = NEBLPercentEscapedStringFromString([NSString stringWithFormat:@"%@",obj]);
            ...
            NSString * safeValue = NEBLPercentEscapedStringFromString([values componentsJoinedByString:separatorStyle]);
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, safeValue]];
        } else {
            NSString * safeValue = NEBLPercentEscapedStringFromString([NSString stringWithFormat:@"%@",queryParam]);
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, safeValue]];
        }
        counter += 1;
    }
...
- (void) updateHeaderParams:(NSDictionary * *)headers queryParams:(NSDictionary * *)querys WithAuthSettings:(NSArray *)authSettings {
    ...
    id<NEBLConfiguration> config = self.configuration;
